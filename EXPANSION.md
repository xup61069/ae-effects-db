...
| 74 | BOOTH After Effects 新品候選逐頁核實（PR #129） | **Oops、RainyDay、CrossStreak**；逐頁確認官方 BOOTH 商品頁的 AE host 與功能後加入 `booth.jsonl`；同批修復 `sort.js` 排序並列時的 tie-break 錯誤 | — | 3 筆新增；最新總筆數 2,998 |
| 75 | BOOTH After Effects 掃描延續（PR #130） | —；本輪掃描結果均已收錄或已有略過記錄 | 36 筆記入 `curation/skipped.tsv` | 0 筆新增；最新總筆數 2,998 |
| 76 | 官方廠商線交叉核實：aescripts sitemap 收尾、Gumroad 幾近詳盡（除 1 筆無法查證的 listing）、Boris FX／Digital Anarchy 全數已覆蓋確認、NewBlueFX 官方產品線核實（PR #131） | **NewBlueFX Titler Pro 7**；官方頁確認 After Effects host 與字幕排版功能 | 1 個 Gumroad listing 因資訊不足無法查證而略過 | 1 筆新增；最新總筆數 2,999 |
| 77 | FxFactory After Effects 產品線、Rowbyte、Superluminal 官方頁複查；BOOTH「After Effects」搜尋前 4 頁交叉核實 | —；FxFactory `/products/aftereffects/` 列出的全部產品均已存在資料庫；Rowbyte 7 項與 Superluminal Stardust 官方頁確認無新產品；BOOTH 候選（SelectiveAdjustment、Random Placement、HL_Ezing、ColorEase、Parallaxen、Rand_Pos、Keyframe Master、Shy Group 等）均已有 `skipped.tsv` 決策記憶 | CineFlare 系列 7 項逐頁核實 Works in 僅列 Final Cut Pro（分類頁列表誤標為支援 AE）；Look Sweet 官方頁僅廣告詞無具體功能；Accentize 三項為音訊外掛非視覺效果；BOOTH id 6872180（MultiSlicer 由 361do 重新上架）與既有 `installed.jsonl` 的 MultiSlicer 為同一 GitHub 專案，判定重複 | 0 筆新增；12 筆記入 `curation/skipped.tsv`；最新總筆數 2,999（不變） |
| 78 | 重審另一輪並行擴充提出的 BOOTH 候選（S0_PathfinderX、AEGroupPanel、Layer Splitter、SwayMate、Audio Sculptor）；找不到可查證的官方頁，未強行收錄。改以 `tools/scan_booth.py`／`tools/find_new.py` 掃描新一輪候選，並發現 `curation/skipped.tsv` 有多行因缺換行把多筆決策黏在同一實體行，導致掃描器誤判為「未決」而重覆列出已有決策的候選（已將此檔案修復列為獨立後續任務）；逐一以 URL 交叉確認後，HL_Ezing／ColorEase／Parallaxen／Rand_Pos／Keyframe Master／Shy Group 等均已有正確略過記憶。aescripts sitemap 差異掃描發現 **Aurum（YCraft）** 官方頁已恢復上架（`Add to Cart`、$59.99，無「discontinued」標示），與舊有「已停售」略過記錄矛盾，改為移除該筆略過記錄並重新收錄 | **Aurum**：GPU 加速金屬質感外掛，PBR 材質、HDRI 環境反射與 Smart Edge 細節偵測，官方頁確認現行販售並補英／日翻譯 | 其餘 sitemap 候選（colorlibrary、mate-for-premiere、multiview、rigfast-web-browsers、rigomator、shareio、shape-up、squall、spiderweb-c4d、stack-the-bend-c4d、styler-durden-dctl、textyle-premiere、true-exif、unwarp-for-photoshop、yuvsoft-geometry-corrector 等）均已有停售／非 AE host／既有覆蓋等略過記憶，不重複新增 | 1 筆新增（Aurum 由略過改列收錄）；最新總筆數 3,008 |
| 79 | 修復 `curation/skipped.tsv` 遺失換行的維護輪 | —；找出 348 行因缺換行而把多筆決策黏在同一實體行（每行 3–9 個 tab 欄位，正常應只有 2 欄）；以 slug 為 `[A-Za-z0-9][A-Za-z0-9_.]*(?:[-/][A-Za-z0-9_.]+)*$` 樣式比對每個黏貼欄位尾端、切回各自的 slug／reason，還原成一行一筆決策，未更動任何 slug 或理由文字內容；還原後 348 行拆成 3,003 行，`find_new.py`／`scan_booth.py` 讀取 `skipped.tsv` 第一欄的邏輯已能正確辨識所有先前黏貼在同一行、被忽略的 slug（尤其 `booth-` 開頭項目，現有 1,317 筆）。本輪未搜尋新候選 | — | 0 筆新增；本輪為純維護；資料本身總筆數不變（3,010） |
| 80 | skipped.tsv 換行修復後重跑 `tools/scan_booth.py --pages 12`／`tools/find_new.py --limit 50`，逐一核實掃描出的 199 個候選 | —；aescripts `find_new.py` 待評估 0（sitemap 已無新候選）；BOOTH 199 個候選逐一核實標題與官方頁，均為配信背景影片素材（動画素材屋HIRO 等）、AEP 作品／習作專案檔（isshin、SHIROTA.、ichikawa213292 等）、動態文字／轉場預設包（grape design 等）或教學電子書（ROLLCHECK）；「楽譜から簡単歌詞字幕／Score to Lyric」（k4a - shop）雖是含 AE 腳本的完整工具，但官方頁無可查證日期而略過 | — | 0 筆新增；199 筆記入 `curation/skipped.tsv`；最新總筆數 3,010（不變） |
| 81 | 效果配方擴充：以「常見畫面感＋資料庫既有外掛」為準新增配方，stack 只引用資料庫已收錄外掛（Trapcode Shine／Particular、Universe Light Rays／Camera Shake、S_Cartoon／S_Shake／S_Flicker系列、BCC+ Rays／Primatte Studio／Spill Remover／Light Wrap／Looks／Selective Saturation／Displacement Map／Ripple／Film Stocks／Glow／Cartooner／Camera Shake、Primatte Keyer、Saber、Optical Flares、Real Lens Flares、Twixtor、Impact Lines、Keylight 等） | **17 筆配方**：命名配方 11（上帝光／體積光束、綠幕去背合成、多巴胺糖果色調、液態金屬文字、霓虹招牌閃爍、漫畫集中線／速度線、子彈時間／時間凍結、鏡頭光暈轉場、卡通化／賽璐璐風、色彩彈出／保留強調色、手持晃動／偽手持鏡頭）；內建配方 6（下三分之一字幕條、打字機字幕、3D 翻轉圖卡、星空穿越、景深對焦轉移、操作演示介面，沿用 15 種動畫變體模板）。全部附人工撰寫的英／日翻譯與 look 翻譯；內建配方另含 look 與 variants | 既有「內建配方」系列 15 筆已覆蓋全像投影、液態玻璃、漫畫網點、熱感應、水墨暈染、故障訊號、夢境柔焦、鉛筆素描、金屬浮雕、粒子消散、萬花筒、紙張撕裂、夜視、深度霧化、發光描邊，未重複 | 17 筆新增（recipe 35→52）；另修正 `tools/add.py` 讓 recipe 免日期檢查（與 validate.py 對 recipes.jsonl 的永久豁免一致）；最新總筆數 3,027 |
| 82 | 每日候選掃描 Issue 待評估項目（aescripts 4 個候選＋Gumroad 8 筆）逐頁核實 | **Bounce Motion、Flowr、Highlighter**；Bounce Motion（Laurence）v1.0（2026-08-17）以點擊起點與落點快速建立彈跳動畫並輸出關鍵影格或表達式；Flowr（irrealix／Ilya Sinitsyn）v1.0.0（2026-08-17）GPU 加速從文字／Alpha／OBJ 表面長出受力場導向的漸層絲線；Highlighter（Ahmed Abdallah）v1.0.0（2026-08-18）一鍵建立隨圖層縮放回應的動畫高亮框與游標並可匯出 MOGRT。三筆均附英／日翻譯與 look 翻譯；`classify_kind.py` 誤把 Flowr 判成 script（「aescripts」含 script 子字串），已加入 KIND_OVERRIDES 保持 plugin | radial-menu-for-cinema-4d（Cinema 4D 專用外掛，非 AE host）；Gumroad 8 筆：SaaS Kit（官方頁無可查證 released／updated 日期且侷限 SaaS demo 利基）、Avangers Doomsday Countdown（免費倒數專案模板）、4 筆 Mediterranean 食譜指南（與 AE 無關）、KOTE BETA 沿用既有 `kote-beta-kozo` 略過決策並補 `gumroad-bcvbdy` slug 供掃描器辨識 | 3 筆新增；8 筆略過；最新總筆數 3,030 |
| 83 | 分支整理：12 個舊工作分支內容確認已全數被 main 吸收（逐筆以 URL／name 比對資料、skipped 零缺漏），直接合併反而會倒退舊版 dist／EXPANSION，因此不合併、僅刪除分支；另以 BOOTH item URL 交叉比對 `data/*.jsonl` 與 `curation/skipped.tsv`，掃出 23 筆收錄後未同步移除的略過記錄 | — | 23 筆已收錄 slug 移出 `curation/skipped.tsv`（booth-8654824、booth-7726008、booth-7266089、booth-8561858、booth-8580859、booth-8529162、booth-8092692、booth-8547196、booth-8553101、booth-8547494、booth-7955064、booth-7837396、booth-8475992、booth-8683443、booth-8369118、booth-8705527、booth-8618241、booth-8133054、booth-8620753、booth-8237538、booth-8016864、booth-8603133、booth-5892231） | 0 筆新增；最新總筆數 3,030（不變） |
